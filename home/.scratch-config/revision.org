...
(setq custom-theme-directory (concat (file-name-as-directory user-emacs-directory) "lisp"))
...
(general-define-key "C-x C-d" 'dired)
  ...
  (setq fontaine-presets
        `((comic-code
          ...
          (jetbrains-mono
           ...
           :default-weight light) ;; same deal with the weights sort of
          (apple
          ...
          (iosevka-variable
           ...
           :variable-pitch-family "Iosevka Aile" ;; sans-serif
           ;; :variable-pitch-family "Iosevka Etoile" ;; slab-serif
           )
          ...
          (ellograph
           :default-family "Ellograph CF"
           :default-weight light
           :line-spacing 2)
          (sometype
           :default-family "Sometype Mono"
           :default-height ,(+ eh/base-font-height 10))
...
(defun eh/org-update-modus-theme ()
  (modus-themes-with-colors
    ;; this new set of colors is only tested with operandi-tinted
    ...
    (set-face-attribute 'eh/org-keyword-todo nil
                        :background green-intense)
    (set-face-attribute 'eh/org-keyword-next nil
                        :foreground gold
                        :background bg-graph-yellow-0)
    ...
    (set-face-attribute 'eh/org-keyword-question nil
                        :background blue-warmer)
    (set-face-attribute 'eh/org-keyword-idea nil
                        :background magenta-warmer)
...
(use-package caves-of-qud-theme
  :straight (:type git :host github :repo "duien/caves-of-qud-theme"))
...
(use-package marginalia
  ...
  (marginalia-mode))
#+end_src
*** Experiment with autocompletion in buffers
I've started using hippie-expand sometimes. What if I actually used some sort of autocomplete framework?
#+begin_src elisp
;; (use-package corfu
;;   :custom
;;   (corfu-auto t)
...
Add a confirmation message to the new ~elisp-eval-region-or-buffer~ function that has bitten me a few times. It's default binding of =C-c C-e= is too similar to =C-x C-e= and evaluating my entire scratch buffer is _not_ a good idea.
#+begin_src elisp
(defun confirm-eval (&optional ARG PRED)
  (interactive)
  (if (use-region-p)
      (yes-or-no-p "Evaluate region?")
    (yes-or-no-p "Evaluate buffer?")))
(advice-add 'elisp-eval-region-or-buffer :before-while 'confirm-eval)
#+end_src

Show a side window with imenu entries for current buffer
#+begin_src elisp
...
;; something isn't working in the autoload for this and is breaking other things
...
#+begin_src elisp :tangle no :noweb-ref org-preface
...
(defun eh/org-update-theme ()
  ...
  ;; set system appearance (light/dark) based on color theme for readability
  ...
  (pcase (car custom-enabled-themes)
    ...
    ('caves-of-qud
     (message "Setting up org faces for caves-of-qud")
     (set-face-attribute 'eh/org-keyword-todo nil
                         :background "#009403")
     (set-face-attribute 'eh/org-keyword-idea nil
                         :background "#b154cf")
...
(use-package org
  ...
  ;; we have the hook we need now, but redefining keyword symbols doesn't work
  ;; without restarting org-mode
  ;; (fontaine-set-preset . eh/define-org-keywords)
  ;; this needs to be updated for newer modus versions
  ;; (modus-themes-after-load-theme . eh/org-update-modus-theme)
  )
#+end_src
** Making org prettier
